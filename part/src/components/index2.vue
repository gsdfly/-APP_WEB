<template>
  <div class="jo-index" v-show="showHtml">
    ...
    <div class="main">
      <div>
        <div class="activitys" v-if="coupons.length>0">
          <div v-for="(item,index) in coupons" v-if="item.coupon != void 0">
            <img v-if="item.coupon.status == 0" :src="imgs[index].img_receive" alt=""
                 @click="consumer(index,item.coupon.id,item.coupon.type)"/>
            <img v-else-if="item.coupon.status == 1" :src="imgs[index].img_use" alt=""
            ...
            <img v-else="" :src="imgs[index].img_already_use" @click="useInfinite(item.coin_price.type)" alt="">
            <span v-if="item.coin_price.type != 1 " class="quan_game_num">{{item.coin_price.coin_num/info.coin_num}}次</span>
            <span v-if="item.coin_price.type == 1" class="cost-price">{{item.coin_price.coin_price}}</span>
            <span style="pointer-events: none;" class="infinity-tip" v-if="item.coin_price.type == 1 && item.coupon.type==1">{{(item.coin_price.coin_price - item.coupon.reduce).toFixed(1)}}元无限抓</span>
            <span style="pointer-events: none;" class="infinity-tip" v-if="item.coin_price.type == 1 && item.coupon.type==0">{{(item.coin_price.coin_price*item.coupon.reduce/100).toFixed(1)}}元无限抓</span>
        ...
        <div class="center">
          ...
          <button class="startgame" :class="{'hasclick':start_desc == '投币中'}" id="coin-operated"
                  @click="handleStartingDevice">{{start_desc}}
          </button>
    ...
    <div class="bg" v-show="bgShow">
      ...
      <div class="bg-center1" v-if="contentShow == 'infinityTip'">
        ...
        <button class="btnTip bg-center-btn3" @click="useCoupons(1,coupons[1].coupon.id,coupons[1].coin_price.coin_price_id,coupons[1].coupon.type,coupons[1].coin_price.coin_num,coupons[1].coin_price.type)"></button>
        <img src="http://res.catchme.com.cn/imgs-2017-12-29-20-42/icon_close.png" alt="" class="close" @click="closeBg">
        <span v-if="coupons[1].coin_price.type == 1" class="cost-price-bg">{{coupons[1].coin_price.coin_price}}</span>
        <span style="pointer-events: none;" class="infinity-tip-bg" v-if="coupons[1].coin_price.type == 1 && coupons[1].coupon.type==1">{{(coupons[1].coin_price.coin_price - coupons[1].coupon.reduce).toFixed(1)}}元无限抓</span>
        <span style="pointer-events: none;" class="infinity-tip-bg" v-if="coupons[1].coin_price.type == 1 && coupons[1].coupon.type==0">{{(coupons[1].coin_price.coin_price*coupons[1].coupon.reduce/100).toFixed(1)}}元无限抓</span>
      </div>
  ...
  export default {
    data() {
      return {
        ...
        imgs: [
          {
            img_receive: 'http://res.catchme.com.cn/imgs-2017-12-29-20-42/quan_free_un.png',
            img_use: 'http://res.catchme.com.cn/imgs-2017-12-29-20-42/quan_free.png',
            img_already_use:'http://res.catchme.com.cn/imgs-2017-12-29-20-42/quan_free_used.png'
          },
          {
            img_receive: 'http://res.catchme.com.cn/imgs-2017-12-29-20-42/quan_20.png',
            img_use: 'http://res.catchme.com.cn/imgs-2017-12-29-20-42/quan_20_use.png',
            img_already_use: 'http://res.catchme.com.cn/imgs-2017-12-29-20-42/quan_card_20_used.png'
          },
        ]
      }
    },
    ...
    methods: {
      ...
      //领取优惠券
      consumer(index, coupon_id,coupon_type) {
      ...
      useCoupons(index, coupon_id, coin_price_id, coupon_type, coin, type) {
        if (coupon_type == 2) {
          this.$store.dispatch('getFreeCoin', {coin_price_id: coin_price_id, coupon_id: coupon_id}).then((data) => {
            ...
            this.coupons[index].coupon.status = 2;
            this.bgShow = false;
          });
        } else {
          var self = this;
            ...
            this.$store.dispatch('getPayAlipay', {coin_price_id: coin_price_id, coupon_id: coupon_id}).then((data) => {
              AlipayJSBridge.call("tradePay", {
                ...
                if (result.resultCode == '9000') {
                  ...
                  self.coupons[index].coupon.status = 2;
                }
              });
            })
          } else if (CONFIG.isWx) {
